...
## LoRA vs Full Fine-Tuning (what we do)
- **LoRA (low-rank adapters)**: Adds small trainable matrices; base weights stay frozen. Benefits: fits on 16 GB with 4-bit loading, fast to train, easy to swap adapters, low risk of catastrophic drift. This is the path used here (4-bit + LoRA via Unsloth/PEFT).
- **Full fine-tune**: Updates all weights. Benefits: maximum capacity to adapt; costs: much higher VRAM/compute (FP16 7B generally needs >24 GB for training), slower, risk of overfitting/drift. Not recommended on a 16 GB GPU.
- **Recommendation**: Stay with 4-bit + LoRA for both 3B and 7B on 16 GB. If you need full fine-tune, use larger GPUs and strong regularization/checkpoints.
